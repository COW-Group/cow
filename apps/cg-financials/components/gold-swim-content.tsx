...
function GoldSwimContentInner({ userEmail }: GoldSwimContentProps) {
  ...
  const defaultTotalUnitSubscription = (((spotPriceEUR) / 31.1034768 + 15) / (1 - PARAMETERS.transactionBrokerage)) * 1000
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      case "charts":
        return <QuarterlyGrowthChart initialInvestment={initialInvestment} />
      case "table":
        return <QuarterlyProjectionTable initialInvestment={initialInvestment} />
      default:
        return <GoldSwimOverviewSection />
    }
  }
