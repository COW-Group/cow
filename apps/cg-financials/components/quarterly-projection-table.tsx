...
import { calculateAllQuarters } from "@/lib/gold-swim-calculations"

interface QuarterlyProjectionTableProps {
  initialInvestment: number
}

export default function QuarterlyProjectionTable({ initialInvestment }: QuarterlyProjectionTableProps) {
  ...
  // Calculate all quarters dynamically based on live price and user input
  const quarterlyData = useMemo(() => {
    return calculateAllQuarters(spotPriceEUR, initialInvestment, 25)
  }, [spotPriceEUR, initialInvestment])
      ...
      <div className="border-2 border-blue-200 bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div className="px-4 sm:px-6 py-6 border-b border-blue-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-gradient-to-br from-emerald-50 to-emerald-100/50 rounded-lg p-4 border-2 border-emerald-200 shadow-sm">
              <p className="text-xs font-semibold text-emerald-700 mb-2">Initial Investment</p>
        ...
        <div className="p-4 sm:p-6">
          <div className="overflow-x-auto rounded-lg border border-gray-300 shadow-sm">
            <table className="text-xs border-collapse table-fixed" style={{ width: '3280px' }}>
              <thead>
                <tr className="bg-gradient-to-r from-blue-600 to-cyan-500">
                  <th className="text-center py-2 px-2 font-semibold text-white sticky left-0 bg-gradient-to-r from-blue-600 to-cyan-500 w-[40px] z-10 shadow-md">Q</th>
              ...
              <tbody>
                {displayData.map((row, idx) => (
                  ...
                  >
                    <td className="py-1.5 px-2 font-bold sticky left-0 bg-inherit text-gray-900 w-[40px] z-10 shadow-sm text-center">Q{row.quarter}</td>
