...
import GoldSummaryCard from "@/components/gold-summary-card"
import ParametersTable from "@/components/parameters-table"
import MarginCalculation from "@/components/margin-calculation"
import ExitPriceCalculation from "@/components/exit-price-calculation"
import ContractValueGain from "@/components/contract-value-gain"
import SummaryTable from "@/components/summary-table"
import DetailedNotes from "@/components/detailed-notes"
import { FileText, Calculator, TrendingUp, DollarSign, Table, FileSpreadsheet, ChevronRight, Menu } from "lucide-react"
...
type Section = "overview" | "parameters" | "margin" | "exit-price" | "contract-value" | "summary" | "notes"

const sections = [
  { id: "overview" as Section, label: "Overview", icon: FileSpreadsheet },
  { id: "parameters" as Section, label: "Assumptions", icon: FileText },
  { id: "margin" as Section, label: "Margin Calculation", icon: Calculator },
  { id: "exit-price" as Section, label: "Exit Price Calculation", icon: TrendingUp },
  { id: "contract-value" as Section, label: "Contract Value & Gain", icon: DollarSign },
  { id: "summary" as Section, label: "Summary Table", icon: Table },
  { id: "notes" as Section, label: "Detailed Notes", icon: FileSpreadsheet },
]
...
export default function DashboardContent({ userEmail }: DashboardContentProps) {
  ...
  const renderSection = () => {
    switch (activeSection) {
      case "overview":
        return <GoldSummaryCard />
      case "parameters":
        return <ParametersTable />
      case "margin":
        return <MarginCalculation />
      case "exit-price":
        return <ExitPriceCalculation />
      case "contract-value":
        return <ContractValueGain />
      case "summary":
        return <SummaryTable />
      case "notes":
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-cyan-50">
        ...
        <div className="container mx-auto px-3 sm:px-4 pt-4 sm:pt-6 max-w-7xl">
          <nav className="flex items-center text-xs sm:text-sm overflow-x-auto mb-4 sm:mb-6">
            ...
            <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 mx-1 sm:mx-2 text-gray-400 flex-shrink-0" />
            <span className="text-gray-700 font-medium whitespace-nowrap">SiriZ31</span>
          </nav>
        </div>
