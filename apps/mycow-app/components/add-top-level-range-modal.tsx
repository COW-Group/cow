...
import { addRange } from "@/app/actions/wealth-vision-actions" // Import the server action for top-level ranges
import { useToast } from "@/hooks/use-toast"

interface AddTopLevelRangeModalProps {
  isOpen: boolean
  onClose: () => void
}

export function AddTopLevelRangeModal({ isOpen, onClose }: AddTopLevelRangeModalProps) {
  ...
  const handleSave = async () => {
    ...
    startTransition(async () => {
      try {
        const newRange = await addRange(title) // Call the addRange server action
        if (newRange) {
        ...
        toast({ title: "Error", description: "Failed to add range: " + err.message, variant: "destructive" })
      }
    })
  }
      ...
      <DialogContent className="sm:max-w-[425px] bg-cream-50 dark:bg-ink-800 text-ink-800 dark:text-cream-100 border-cream-200 dark:border-ink-700 rounded-xl shadow-lg">
        ...
        <DialogFooter>
          <Button onClick={handleSave} disabled={isPending} className="bg-logo-blue hover:bg-blue-700 text-white">
            {isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Add Range"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
