...
import { updateRange } from "@/app/actions/wealth-vision-actions"
import type { VisionBoardSection } from "@/types/wealth-vision"

interface EditRangeModalProps {
  isOpen: boolean
  onClose: () => void
  range: VisionBoardSection | null
}

export function EditRangeModal({ isOpen, onClose, range }: EditRangeModalProps) {
  const [newTitle, setNewTitle] = useState(range?.name || "")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!range || !newTitle.trim()) {
      toast({
        title: "Error",
        description: "Range title cannot be empty.",
        variant: "destructive",
      })
      return
    }
    ...
    startTransition(async () => {
      const updatedRange = await updateRange(range.id, newTitle.trim())
      if (updatedRange) {
        toast({
          title: "Success",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] bg-cream-50 dark:bg-ink-800 text-ink-800 dark:text-cream-100 border-cream-200 dark:border-ink-700">
        <DialogHeader>
          <DialogTitle className="text-ink-800 dark:text-cream-100">Edit Range</DialogTitle>
          <DialogDescription className="text-ink-600 dark:text-cream-300">
            Make changes to your range here. Click save when you're done.
          </DialogDescription>
        </DialogHeader>
