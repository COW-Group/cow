import type React from 'react'
import { useState, useEffect } from 'react'
import { motion } from 'framer-motion'
import { CheckCircle2, XCircle } from 'lucide-react'
import { useComprehensiveOnboarding } from '@/contexts/comprehensive-onboarding-context'
import { useGuideSpeech } from '@/hooks/use-guide-speech'

export const Step3AgeExperience: React.FC = () => {
  const { state, updateState, nextStep, prevStep } = useComprehensiveOnboarding()
  const [isAdult, setIsAdult] = useState<boolean | null>(state.isAdult)
  const [hasInvestedBefore, setHasInvestedBefore] = useState<boolean | null>(state.hasInvestedBefore)
  const [isSpeaking, setIsSpeaking] = useState(false)

  const introText = `Let's understand your starting point, ${state.firstName}. This helps us tailor your wealth journey to your needs.`

  const { speak, stop } = useGuideSpeech({
    autoPlayText: introText,
    autoPlayGuideType: state.guideType,
    onSpeechStart: () => setIsSpeaking(true),
    onSpeechEnd: () => setIsSpeaking(false),
  })

  useEffect(() => {
    return () => stop()
  }, [stop])

  const handlePrevious = () => {
    stop()
    prevStep()
  }

  const handleNext = () => {
    if (isAdult === null || hasInvestedBefore === null) return
    stop()
    updateState('isAdult', isAdult)
    updateState('hasInvestedBefore', hasInvestedBefore)
    nextStep()
  }

  const isFormValid = isAdult !== null && hasInvestedBefore !== null

  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      exit={{ opacity: 0, y: -20 }}
      transition={{ duration: 0.5 }}
      className="flex flex-col items-center justify-center min-h-screen p-4 md:p-8"
    >
      <div className="w-full max-w-4xl">
        {/* Header */}
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.2, duration: 0.5 }}
          className="text-center mb-8 md:mb-12"
        >
          <h1 className="text-3xl md:text-4xl font-light text-slate-800 dark:text-white mb-4">
            Let's understand your starting point
          </h1>
          <p className="text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto">
            This helps us tailor your wealth journey to your needs
          </p>
        </motion.div>

        {/* Main Content Card */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.3, duration: 0.5 }}
          className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-8 md:p-12 rounded-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg"
        >
          {/* Age Question */}
          <div className="mb-10">
            <h3 className="text-xl font-semibold text-slate-800 dark:text-white mb-6">
              Are you 18 or older?
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <motion.button
                onClick={() => setIsAdult(true)}
                whileHover={{ scale: 1.02, y: -2 }}
                whileTap={{ scale: 0.98 }}
                className={`
                  relative p-6 rounded-xl border-2 transition-all duration-300 text-left
                  ${
                    isAdult === true
                      ? 'border-[#0066FF] bg-gradient-to-br from-[#0066FF]/10 to-transparent shadow-lg shadow-[#0066FF]/20'
                      : 'border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 hover:border-[#0066FF]/50'
                  }
                `}
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`
                    p-3 rounded-lg transition-all
                    ${
                      isAdult === true
                        ? 'bg-gradient-to-br from-[#0066FF] to-[#2563eb]'
                        : 'bg-slate-100 dark:bg-slate-700/50'
                    }
                  `}
                  >
                    <CheckCircle2 className={`w-6 h-6 ${isAdult === true ? 'text-white' : 'text-slate-400'}`} />
                  </div>
                  <span className="text-lg font-medium text-slate-800 dark:text-white">Yes, I'm 18 or older</span>
                </div>
              </motion.button>

              <motion.button
                onClick={() => setIsAdult(false)}
                whileHover={{ scale: 1.02, y: -2 }}
                whileTap={{ scale: 0.98 }}
                className={`
                  relative p-6 rounded-xl border-2 transition-all duration-300 text-left
                  ${
                    isAdult === false
                      ? 'border-[#0066FF] bg-gradient-to-br from-[#0066FF]/10 to-transparent shadow-lg shadow-[#0066FF]/20'
                      : 'border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 hover:border-[#0066FF]/50'
                  }
                `}
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`
                    p-3 rounded-lg transition-all
                    ${
                      isAdult === false
                        ? 'bg-gradient-to-br from-[#0066FF] to-[#2563eb]'
                        : 'bg-slate-100 dark:bg-slate-700/50'
                    }
                  `}
                  >
                    <XCircle className={`w-6 h-6 ${isAdult === false ? 'text-white' : 'text-slate-400'}`} />
                  </div>
                  <span className="text-lg font-medium text-slate-800 dark:text-white">No, I'm under 18</span>
                </div>
              </motion.button>
            </div>
          </div>

          {/* Investment Experience Question */}
          {isAdult === true && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4 }}
              className="mb-8"
            >
              <h3 className="text-xl font-semibold text-slate-800 dark:text-white mb-6">
                Have you invested before?
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <motion.button
                  onClick={() => setHasInvestedBefore(true)}
                  whileHover={{ scale: 1.02, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  className={`
                    relative p-6 rounded-xl border-2 transition-all duration-300 text-left
                    ${
                      hasInvestedBefore === true
                        ? 'border-[#059669] bg-gradient-to-br from-[#059669]/10 to-transparent shadow-lg shadow-[#059669]/20'
                        : 'border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 hover:border-[#059669]/50'
                    }
                  `}
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`
                      p-3 rounded-lg transition-all
                      ${
                        hasInvestedBefore === true
                          ? 'bg-gradient-to-br from-[#059669] to-[#047857]'
                          : 'bg-slate-100 dark:bg-slate-700/50'
                      }
                    `}
                    >
                      <CheckCircle2
                        className={`w-6 h-6 ${hasInvestedBefore === true ? 'text-white' : 'text-slate-400'}`}
                      />
                    </div>
                    <div>
                      <span className="text-lg font-medium text-slate-800 dark:text-white block">Yes</span>
                      <span className="text-sm text-slate-600 dark:text-slate-400">
                        I have investment experience
                      </span>
                    </div>
                  </div>
                </motion.button>

                <motion.button
                  onClick={() => setHasInvestedBefore(false)}
                  whileHover={{ scale: 1.02, y: -2 }}
                  whileTap={{ scale: 0.98 }}
                  className={`
                    relative p-6 rounded-xl border-2 transition-all duration-300 text-left
                    ${
                      hasInvestedBefore === false
                        ? 'border-[#059669] bg-gradient-to-br from-[#059669]/10 to-transparent shadow-lg shadow-[#059669]/20'
                        : 'border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-800/50 hover:border-[#059669]/50'
                    }
                  `}
                >
                  <div className="flex items-center gap-4">
                    <div
                      className={`
                      p-3 rounded-lg transition-all
                      ${
                        hasInvestedBefore === false
                          ? 'bg-gradient-to-br from-[#059669] to-[#047857]'
                          : 'bg-slate-100 dark:bg-slate-700/50'
                      }
                    `}
                    >
                      <CheckCircle2
                        className={`w-6 h-6 ${hasInvestedBefore === false ? 'text-white' : 'text-slate-400'}`}
                      />
                    </div>
                    <div>
                      <span className="text-lg font-medium text-slate-800 dark:text-white block">No</span>
                      <span className="text-sm text-slate-600 dark:text-slate-400">I'm just getting started</span>
                    </div>
                  </div>
                </motion.button>
              </div>
            </motion.div>
          )}
        </motion.div>

        {/* Navigation Buttons */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.4, duration: 0.3 }}
          className="mt-8 max-w-2xl mx-auto"
        >
          <div className="flex gap-4">
            <button
              onClick={handlePrevious}
              className="flex-shrink-0 px-8 py-6 rounded-xl border-2 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 font-semibold text-lg hover:border-[#0066FF] hover:text-[#0066FF] dark:hover:text-[#38bdf8] transition-all duration-300"
            >
              Previous
            </button>

            <button
              onClick={handleNext}
              disabled={!isFormValid}
              className="flex-1 bg-gradient-to-r from-[#0066FF] to-[#2563eb] hover:from-[#2563eb] hover:to-[#059669] text-white font-semibold text-lg py-6 rounded-xl shadow-lg shadow-[#0066FF]/25 hover:shadow-xl hover:shadow-[#2563eb]/30 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:from-[#0066FF] disabled:hover:to-[#2563eb]"
            >
              Continue
            </button>
          </div>
        </motion.div>
      </div>
    </motion.div>
  )
}
