      ...
      <div className="flex flex-col items-center">
        ...
        <div className="w-full max-w-4xl mt-8 flex flex-col lg:flex-row gap-8">
          ...
          <motion.div
            ...
            className="flex-1 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-6 rounded-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg"
          >
            <h3 className="text-xl font-light text-slate-800 dark:text-slate-100 mb-4">Select Your Quests</h3>
            ...
            <ScrollArea className="h-[320px] pr-4">
              <div className="space-y-3">
                {state.lifeGoals.map((goal) => (
                  <div
                    key={goal.id}
                    className="flex items-center space-x-3 rounded-lg border border-slate-200 dark:border-slate-700 p-3 cursor-pointer hover:bg-slate-100/50 dark:hover:bg-slate-700/50 transition-colors"
                  ...
                  >
                    ...
                    <Label
                      ...
                      className="flex-1 cursor-pointer font-light text-slate-800 dark:text-slate-100"
                    >
                      {goal.name}
                    </Label>
                  </div>
            ...
            <div className="mt-4 flex items-center space-x-2">
              <Input
                ...
                className="bg-slate-100/70 dark:bg-slate-700/70 border-slate-200/70 dark:border-slate-600/70"
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    e.preventDefault()
                    handleAddCustomGoal()
              ...
              <button
                ...
                className="px-4 py-2 bg-[#0066FF] text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Add
              </button>
            </div>
          ...
          <motion.div
            ...
            className="flex-1 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm p-6 rounded-xl border border-slate-200/50 dark:border-slate-700/50 shadow-lg relative"
          >
            <h3 className="text-xl font-light text-slate-800 dark:text-slate-100 mb-4">How can MyCOW help you?</h3>
                ...
                <div
                  ...
                  className={`flex items-center space-x-3 rounded-lg border p-3 cursor-pointer transition-colors ${
                    state.myCowHelp.includes(help as any)
                      ? "border-[#0066FF] bg-blue-50/50 dark:bg-blue-900/20"
            ...
            {showQuestMap && (
              ...
              >
                <div className="bg-slate-100 dark:bg-slate-700 rounded-full p-3 shadow-lg">
                  <MapPin className="w-6 h-6 text-[#0066FF] dark:text-blue-400" />
                </div>
              </motion.div>
            )}
          </motion.div>
