  ...
  return (
    <div className="max-w-4xl mx-auto w-full p-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-xl shadow-lg border border-slate-200/50 dark:border-slate-700/50 mb-20">
      ...
      >
        ...
        <div>
          <Label htmlFor="country-select" className="text-lg font-medium text-slate-700 dark:text-slate-200 mb-4 block">
          ...
          <div className="relative">
            ...
            >
              <SelectTrigger className="pl-10 bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
        ...
        {country && (
          ...
          >
            <Label className="text-lg font-medium text-slate-700 dark:text-slate-200 mb-4 block">
              What is your annual income range? ({currency || "USD"})
            </Label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label htmlFor="min-income" className="text-slate-700 dark:text-white0">
              ...
              <div className="space-y-2">
                <Label htmlFor="max-income" className="text-slate-700 dark:text-white0">
                ...
                <div
                  ...
                  className={`flex items-center space-x-3 rounded-lg border p-4 cursor-pointer transition-all duration-300 ${
                    riskTolerance === risk.value
                      ? "border-[#0066FF] bg-blue-50/50 dark:bg-blue-900/20 shadow-lg"
                      : "border-slate-200 dark:border-slate-700 hover:bg-slate-100/50 dark:hover:bg-slate-700/50"
                  }`}
                >
                  <RadioGroupItem value={risk.value} id={`risk-${risk.value}`} />
                  <div className="flex-1">
                    <Label htmlFor={`risk-${risk.value}`} className="cursor-pointer">
                      <div className="font-medium text-slate-800 dark:text-slate-100">{risk.label}</div>
                      <div className="text-sm text-slate-600 dark:text-slate-300">{risk.description}</div>
                    </Label>
                  </div>
        ...
        {showRiskMeter && riskTolerance && (
          <motion.div
            ...
            transition={{ delay: 0.5 }}
            className="bg-gradient-to-r from-slate-100 to-blue-50 dark:from-slate-800 dark:to-blue-900/20 p-6 rounded-xl border border-blue-200/50 dark:border-blue-800/30"
          >
            <div className="flex items-center justify-center mb-4">
              <Gauge className="w-8 h-8 text-[#0066FF] mr-3" />
              <h3 className="text-lg font-medium text-slate-800 dark:text-slate-100">Your Risk Profile</h3>
            </div>

            <div className="relative">
              <div className="w-full h-4 bg-gradient-to-r from-emerald-500 via-yellow-500 via-orange-500 to-red-500 rounded-full" />
              <motion.div
                className="absolute top-0 w-4 h-4 bg-white border-2 border-slate-800 dark:border-slate-100 rounded-full shadow-lg"
                initial={{ left: "0%" }}
