...
import { Calculator, Lock, Table, BarChart3, FileSpreadsheet, ArrowLeft, Shield, ChevronRight, Info } from "lucide-react";
import cowLogo from "../assets/cow-logo.png";
import { GoldPriceProvider, useGoldPriceContext } from "../contexts/gold-price-context";
import SiriZ31OverviewSection from "../components/siri-z31-overview-section";
import SiriZ31PositionCalculator from "../components/siri-z31-position-calculator";
import SiriZ31ExitAnalysis from "../components/siri-z31-exit-analysis";
import SiriZ31PositionSummary from "../components/siri-z31-position-summary";
import { type SiriZ31Params } from "../lib/gold-price-calculations";

type Section = "overview" | "calculator" | "exit" | "summary"

const sections = [
  { id: "overview" as Section, label: "Overview", icon: FileSpreadsheet },
  { id: "calculator" as Section, label: "Position Calculator", icon: Calculator },
  { id: "exit" as Section, label: "Exit Analysis", icon: BarChart3 },
  { id: "summary" as Section, label: "Position Summary", icon: Table },
]

function InvestSiriZ31Content() {
  const { auth } = useAuthContext();
  const navigate = useNavigate();
  const [isClient, setIsClient] = useState(false);
  const [activeSection, setActiveSection] = useState<Section>("overview");
  const { spotAsk } = useGoldPriceContext();

  // Default position parameters
  const defaultSpotPrice = spotAsk || 2650.00;
  const [positionParams, setPositionParams] = useState<SiriZ31Params>({
    entryPrice: defaultSpotPrice,
    numberOfContracts: 1,
    investmentAmount: 50000,
    targetExitPrice: defaultSpotPrice * 1.15, // +15% target
  });

  // Update entry and exit prices when spot price changes
  useEffect(() => {
    if (spotAsk) {
      setPositionParams(prev => ({
        ...prev,
        entryPrice: spotAsk,
        targetExitPrice: spotAsk * 1.15,
      }));
    }
  }, [spotAsk]);
  ...
  const handleParamChange = (param: keyof SiriZ31Params, value: number) => {
    setPositionParams(prev => ({ ...prev, [param]: value }));
  };
  ...
  const handleReset = () => {
    setPositionParams({
      ...
      numberOfContracts: 1,
      investmentAmount: 50000,
      targetExitPrice: defaultSpotPrice * 1.15,
    });
  };
  ...
  const renderSection = () => {
    switch (activeSection) {
      case "overview":
        return <SiriZ31OverviewSection />;
      case "calculator":
        return (
          <SiriZ31PositionCalculator
      ...
      case "exit":
        return <SiriZ31ExitAnalysis params={positionParams} />;
      case "summary":
        return <SiriZ31PositionSummary params={positionParams} />;
      default:
        return <SiriZ31OverviewSection />;
    }
  };
      ...
      <main className="pt-32 px-8 pb-20">
        <div className="max-w-7xl mx-auto">
          ...
          <nav className="flex items-center text-xs sm:text-sm overflow-x-auto mb-4 sm:mb-6">
            ...
            <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 mx-1 sm:mx-2 text-gray-400 flex-shrink-0" />
            <span className="text-gray-700 dark:text-gray-300 font-medium whitespace-nowrap">SIRI Z31 Calculator</span>
          </nav>
          ...
          <div className="text-center mb-12">
            ...
            >
              ...
              <h1 className="text-5xl sm:text-6xl font-light mb-4 text-gray-900 dark:text-gray-100" style={{ letterSpacing: '-0.02em' }}>
                SIRI Z31 Investment Calculator
              </h1>
              <p className="text-lg font-light text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
                Model your futures-hedged gold investment with systematic positioning analysis
              </p>
            </motion.div>
          </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-blue-800 dark:text-blue-200 mb-3">
                      SIRI Z31 represents 8.33% of the MyGOLD diversified investment allocation and is not available as a standalone investment. This calculator is provided for educational purposes to help you understand the systematic futures positioning component within your MyGOLD investment.
                    </p>
              ...
              <Card className="border-2 border-amber-500/30 bg-amber-50/50 dark:bg-amber-950/20 max-w-2xl mx-auto">
                <CardContent className="pt-12 pb-12 text-center">
                  ...
                  <p className="text-amber-800 dark:text-amber-200 mb-8 max-w-md mx-auto">
                    The SIRI Z31 Investment Calculator is available exclusively to authenticated investors. Sign in or create an account to access professional-grade futures analysis tools.
                  </p>
                ...
                <nav className="bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-blue-200 dark:border-gray-700 overflow-hidden sticky top-24">
                  ...
                  <div className="bg-gradient-to-r from-blue-600 to-cyan-500 p-4">
                    <div className="flex items-center justify-between mb-2">
                      <h2 className="text-2xl font-bold text-white">SIRI Z31</h2>
...
export default function InvestSiriZ31Page() {
  return (
    <GoldPriceProvider>
      <InvestSiriZ31Content />
    </GoldPriceProvider>
  );
}
