...
let maunaVoice: SpeechSynthesisVoice | null = null
...
function loadGuideVoices() {
  ...
  // Load Mauna voice (female)
  maunaVoice = findFemaleVoice(voices)

  voicesLoaded = true
  console.log("loadGuideVoices: Mauna voice selected:", maunaVoice?.name || "None found")
...
// Updated function to support Mauna guide voice (Female) with speech controls
export async function speakWithGuideVoice(
  text: string,
  guideType: "Mauna" | "mauna",
  onEnd?: () => void,
  onError?: (error: string) => void,
  options?: {
    rate?: number
  ...
  const voice = maunaVoice

  console.log(`Selected Mauna voice:`, voice?.name || "default")

  if (!voice) {
    console.warn(`No specific Mauna voice found. Using default browser voice.`)
  }

  speakFullText(text, voice, onEnd, onError, options)
}
...
const speakNextInQueue = () => {
  ...
  const { text, voice, onEnd, onError, rate = 1, pitch = 1, volume = 1 } = speechQueue[0]
  const utterance = new SpeechSynthesisUtterance(text)
  ...
  window.speechSynthesis.speak(utterance)
  console.log(
    `speakNextInQueue: Speaking with voice: "${utterance.voice?.name || "default"}" for text: "${text.substring(0, 50)}..." at rate: ${rate}, pitch: ${pitch}, volume: ${volume}`,
  )
}
...
export const pauseSpeech = () => {
  ...
  if (isSpeaking && !isPaused) {
    ...
    isPaused = true
    console.log("pauseSpeech: Speech paused.")
  }
}
...
export const resumeSpeech = () => {
  ...
  if (isSpeaking && isPaused) {
    ...
    isPaused = false
    console.log("resumeSpeech: Speech resumed.")
  }
}

export const isSpeechPaused = () => isPaused
